...
namespace SpeakToMe
{
	public partial class ViewController : UIViewController, ISFSpeechRecognizerDelegate
	{
		...
		void RecordButtonTapped ()
		{
            try
            {
                if (audioEngine.Running)
                {
                    ...
                    SpeakToMe(TextToSpeak: textView.Text);

                    SpeakToMe(TextToSpeak: "triple 20, triple 17, double 20");
                    textView2.Text = "triple 20, triple 17, double 20";
                    //TextToSpeech.SpeakAsync("End Recording");
                }
                else
                {
                    StartRecording();
        ...
        void SpeakToMe(string TextToSpeak)
        {
            try
            {
                //Routine for Speech to Text
